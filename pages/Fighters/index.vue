<template>

<div class="md:grid grid-cols-4 m-auto">

  <div class="relative boxer-card-container p-6 mb-6 rounded max-w-xs bg-gray-200">

      <div class="boxer-name font-bold pb-2 text-center text-gray-700">
        <h2>Corby Baker</h2>
      </div>

      <div class="absolute top-0.5 mt-3 left-0.5 ml-3 bg-[#3672bb] font-semibold text-white px-3 py-2 rounded-full text-sm">82</div>

      <div class="w-48 h-64 m-auto">
        <img src="https://postimg.cc/HrV7pfFp"
      </div>

      <div>

        <div class="boxer-name font-bold py-2 text-center text-gray-700">
          <h3>@HaughtyOrchid88</h3>
        </div>

        <div class="text-center record gap-1 flex justify-center font-bold text-white">
          
          <div class="wins bg-green-600 py-1 px-6 rounded">
            <p>0</p>
            <span>WINS</span>                 
          </div>

          <div class="losses bg-red-600 p-1 px-6 rounded">
            <p>0</p>
            <span>LOSSES</span>          
          </div>

          <div class="draws bg-blue-400 p-1 px-6 rounded">
            <p>0</p>
            <span>DRAWS</span>          
          </div>

        </div>  

        <div>   
          <div class="font-semibold pt-4 justify-center record flex gap-4">
            <div class="wins text-green-700">
              <p>0<span> KO WINS</span></p>        
            </div>
            <div class="losses text-red-700">
              <p>0<span> KO LOSSES</span></p>
            </div>
          </div>
        </div>

        <div class="pt-4 text-center">
          <p class="font-semibold">Welterweight</p>
          <p>HT- 5' 9", WT- 146 lbs, RCH- 72"</p>
        </div>

        <div class="mt-3 btn2 text-center">View Fighter</div>

      </div>
  </div>

  <div class="relative boxer-card-container p-6 mb-6 rounded max-w-xs bg-gray-200">

<div class="boxer-name font-bold pb-2 text-center text-gray-700">
  <h2>Jacob Mildner</h2>
</div>

<div class="absolute top-0.5 mt-3 left-0.5 ml-3 bg-[#3672bb] font-semibold text-white px-3 py-2 rounded-full text-sm">90</div>

<div class="w-48 h-64 m-auto">
  <img src="https://scontent.ftpa1-2.fna.fbcdn.net/v/t39.30808-6/324398935_1307164923452643_7274313850931195399_n.jpg?_nc_cat=100&ccb=1-7&_nc_sid=730e14&_nc_ohc=WiruHtC2Rr8AX9Swtfx&_nc_ht=scontent.ftpa1-2.fna&oh=00_AfC6EL0WMGvXRhsEu1J4rCtMXbFgxtKRW7TqpAMLybNiDw&oe=63BBCFB6" alt="me">
</div>

<div>

  <div class="boxer-name font-bold py-2 text-center text-gray-700">
    <h3>@WarPappii</h3>
  </div>

  <div class="text-center record gap-1 flex justify-center font-bold text-white">
    
    <div class="wins bg-green-600 py-1 px-6 rounded">
      <p>1</p>
      <span>WINS</span>                 
    </div>

    <div class="losses bg-red-600 p-1 px-6 rounded">
      <p>0</p>
      <span>LOSSES</span>          
    </div>

    <div class="draws bg-blue-400 p-1 px-6 rounded">
      <p>0</p>
      <span>DRAWS</span>          
    </div>

  </div>  

  <div>   
    <div class="font-semibold pt-4 justify-center record flex gap-4">
      <div class="wins text-green-700">
        <p>1<span> KO WINS</span></p>        
      </div>
      <div class="losses text-red-700">
        <p>0<span> KO LOSSES</span></p>
      </div>
    </div>
  </div>

  <div class="pt-4 text-center">
    <p class="font-semibold">Middleweight</p>
    <p>HT- 5' 11", WT- 155 lbs, RCH- 77"</p>
  </div>

  <div class="mt-3 btn2 text-center">View Fighter</div>

</div>
</div>

  <div class="relative boxer-card-container p-6 mb-6 rounded max-w-xs bg-gray-200">

  <div class="boxer-name font-bold pb-2 text-center text-gray-700">
    <h2>Clubber Lang</h2>
  </div>

  <div class="absolute top-0.5 mt-3 left-0.5 ml-3 bg-[#3672bb] font-semibold text-white px-3 py-2 rounded-full text-sm">80</div>

  <div class="w-48 m-auto h-64">
    <img src="https://scontent.ftpa1-2.fna.fbcdn.net/v/t39.30808-6/313197716_185179717410997_9056183693428324314_n.jpg?_nc_cat=102&ccb=1-7&_nc_sid=730e14&_nc_ohc=XCSyH1zwr3sAX_vLcdm&_nc_ht=scontent.ftpa1-2.fna&oh=00_AfDBrqcWXOWVnnXQNH8PlF2h9UdTqcTqkdalHLpFpgThQg&oe=63B7FA73" alt="me">
  </div>

  <div>

    <div class="boxer-name font-bold py-2 text-center text-gray-700">
      <h3>@WING CHUN175879</h3>
    </div>

    <div class="text-center record gap-1 flex justify-center font-bold text-white">
      
      <div class="wins bg-green-600 py-1 px-6 rounded">
        <p>1</p>
        <span>WINS</span>                 
      </div>

      <div class="losses bg-red-600 p-1 px-6 rounded">
        <p>0</p>
        <span>LOSSES</span>          
      </div>

      <div class="draws bg-blue-400 p-1 px-6 rounded">
        <p>0</p>
        <span>DRAWS</span>          
      </div>

    </div>  

    <div>   
      <div class="font-semibold pt-4 justify-center record flex gap-4">
        <div class="wins text-green-700">
          <p>1<span> KO WINS</span></p>        
        </div>
        <div class="losses text-red-700">
          <p>0<span> KO LOSSES</span></p>
        </div>
      </div>
    </div>

    <div class="pt-4 text-center">
      <p class="font-semibold">Middleweight</p>
      <p>HT- 6' 0", WT- 155 lbs, RCH- 75"</p>
    </div>

    <div class="mt-3 btn2 text-center">View Fighter</div>

  </div>


  </div>

  <div class="relative boxer-card-container p-6 mb-6 rounded max-w-xs bg-gray-200">

<div class="boxer-name font-bold pb-2 text-center text-gray-700">
  <h2>Wata Bully</h2>
</div>

<div class="absolute top-0.5 mt-3 left-0.5 ml-3 bg-[#3672bb] font-semibold text-white px-3 py-2 rounded-full text-sm">86</div>

<div class="w-48 h-64 m-auto">
  <img src="https://scontent.ftpa1-2.fna.fbcdn.net/v/t39.30808-6/322961307_551460366653891_708550471661016674_n.jpg?_nc_cat=103&ccb=1-7&_nc_sid=730e14&_nc_ohc=TzVX9FXkUmQAX_Q24wD&_nc_ht=scontent.ftpa1-2.fna&oh=00_AfDC5AXAFkYVbuyW6fFrBexPdpWPzW5V-IElQqtSWQFgrQ&oe=63B7FEDD" alt="me">
</div>

<div>

  <div class="boxer-name font-bold py-2 text-center text-gray-700">
    <h3>@WyteNin9</h3>
  </div>

  <div class="text-center record gap-1 flex justify-center font-bold text-white">
    
    <div class="wins bg-green-600 py-1 px-6 rounded">
      <p>0</p>
      <span>WINS</span>                 
    </div>

    <div class="losses bg-red-600 p-1 px-6 rounded">
      <p>0</p>
      <span>LOSSES</span>          
    </div>

    <div class="draws bg-blue-400 p-1 px-6 rounded">
      <p>0</p>
      <span>DRAWS</span>          
    </div>

  </div>  

  <div>   
    <div class="font-semibold pt-4 justify-center record flex gap-4">
      <div class="wins text-green-700">
        <p>0<span> KO WINS</span></p>        
      </div>
      <div class="losses text-red-700">
        <p>0<span> KO LOSSES</span></p>
      </div>
    </div>
  </div>

  <div class="pt-4 text-center">
    <p class="font-semibold">Middleweight</p>
    <p>HT- 6' 0", WT- 160 lbs, RCH- 78"</p>
  </div>

  <div class="mt-3 btn2 text-center">View Fighter</div>

  </div>

  </div>

</div>
  
</template>

<script setup>

</script>

<style scoped>

</style>

